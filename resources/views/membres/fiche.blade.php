    ...
    <div class="max-w-4xl mx-auto px-4">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex space-x-3">
                <button onclick="window.print()" class="bg-white border border-gray-300 px-4 py-2 rounded-lg text-gray-700 hover:bg-gray-50 flex items-center shadow-sm">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2z"/></svg>
                    Imprimer la fiche
                </button>
                <a href="{{ route('membres.edit', $membre) }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 shadow-sm">
                    Modifier le profil
                </a>
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
            ...
            <div class="h-32 {{ $membre->type_membre == 'Membre effectif' ? 'bg-green-600' : ($membre->type_membre == 'Membre d’honneur' ? 'bg-purple-600' : 'bg-blue-600') }}"></div>

            <div class="px-8 pb-8">
                <div class="relative -mt-16 flex flex-col md:flex-row items-end md:items-center space-y-4 md:space-y-0 md:space-x-6 mb-8">
                    ...
                    <div class="flex-1">
                        <div class="flex items-center space-x-3">
                            ...
                            <span class="px-3 py-1 bg-gray-100 text-gray-600 text-xs font-bold rounded-full uppercase tracking-widest">
                                {{ $membre->numero_membre }}
                            </span>
                        </div>
                        <p class="text-blue-600 font-bold text-lg">{{ $membre->fonction }}</p>
                        <div class="mt-2 inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-opacity-10 
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-6">
                        <h2 class="text-sm font-black text-gray-400 uppercase tracking-widest border-b pb-2">Informations Personnelles</h2>
                        
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs text-gray-400">Date de naissance</label>
                                <p class="text-gray-800 font-semibold">{{ $membre->date_naissance->format('d/m/Y') }}</p>
                            </div>
                            <div>
                                <label class="block text-xs text-gray-400">Lieu de naissance</label>
                                <p class="text-gray-800 font-semibold">{{ $membre->lieu_naissance ?? 'Non renseigné' }}</p>
                            </div>
                            <div>
                    ...
                    <div class="space-y-6">
                        <h2 class="text-sm font-black text-gray-400 uppercase tracking-widest border-b pb-2">Détails Administratifs</h2>
                        ...
                        <div class="space-y-4">
                            <div class="flex items-start space-x-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-800 font-bold">{{ $membre->date_adhesion ? $membre->date_adhesion->format('d F Y') : 'Non définie' }}</p>
                                </div>
                            </div>
                            ...
                            <div class="mt-6 p-4 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                                <div class="flex justify-between items-center">
                                    ...
                                    <a href="{{ asset('storage/' . $membre->piece_jointe) }}" target="_blank" class="text-blue-600 text-sm font-bold hover:underline">Voir le fichier</a>
                                </div>
                            </div>
                            @endif
                        </div>
            ...
            <div class="bg-gray-50 px-8 py-4 border-t border-gray-100 flex justify-between items-center">
                <span class="text-xs text-gray-400 italic font-light italic">Dernière mise à jour : {{ $membre->updated_at->format('d/m/Y H:i') }}</span>
